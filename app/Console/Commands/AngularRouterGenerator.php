...
class AngularRouterGenerator extends Command
{
    /**
     ...
     * @var string
     */
    protected $name = 'angular:generate-api-services';
    ...
    {
        $base = 'api/v1/';
        $routes = array_filter($routes, function (&$route) use ($base) {
            $uri = $route['uri'];
            $route['uri'] = str_replace($base, '', $uri);
            logger($route['uri']);
            return substr($uri, 0, strlen($base)) === $base;
        });
...
export class ";
 ...
 get() {
   return this.httpClient.get('posts').pipe(
